<?php
/**
 * Template for WP Responsive Recent Post Slider Pro Loop - Design-21
 *
 * This template can be overridden by copying it to yourtheme/wp-responsive-recent-post-slider-pro/slider/design-21.php
 *
			...
			<?php if( !empty($featured_img_medium) ) { ?>
				<div class="wprpsp-post-image-wrap wprpsp-post-image-bg">
					<a href="<?php echo esc_url($post_link); ?>" target="<?php echo $link_target; ?>">
						<img <?php if($lazyload) { ?>data-lazy="<?php echo esc_url($featured_img_medium); ?>" <?php } ?> src="<?php if(empty($lazyload)) { echo esc_url($featured_img_medium); } ?>" alt="<?php the_title_attribute(); ?>" />
					</a>
				</div>
			<?php } ?>
			<?php if($show_content) { ?>
